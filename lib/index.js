...
function capabilityNotice(maxUploadBytes, uploadRoots = []) {
	...
	return [
		...
		"- 调用会产出文件的工具/skill 时(生成图片、导出文档等),优先把它的输出目录参数(如 -o/--output)指向会话工作目录,这样产物天然可发送。",
		"- 若产物已经落在允许目录之外(工具默认写 ~/Downloads 之类),先把它复制进会话工作目录,再对复制后的路径写标记——否则会被安全围栏拒绝。",
...
*/
function localUserMessage(content, source) {
...
var SessionBridge = class SessionBridge {
	ctx;
	host;
	store;
	config;
	...
	*/
	userMessage(content, source) {
		const factory = this.host.createUserMessage;
		return factory === void 0 ? localUserMessage(content, source) : factory({
			content,
			source
		});
	}
	...
	*/
	static resolvePreset(header, events) {
...
var QuestionRelay = class {
	...
	stop() {
		for (const [id, waiter] of this.waiters) {
			...
			waiter.reject(/* @__PURE__ */ new Error("discord bridge unloaded"));
		}
	}
	/**
	...
	*/
	async handleAsk(request, next) {
		const sessionId = request.agent?.id;
		const channelId = sessionId === void 0 ? void 0 : this.options.channelForSession(sessionId);
		if (channelId === void 0 || request.questions.length === 0) return await next();
		this.counter += 1;
		...
		const relay = {
			...
			messageIds: request.questions.map(() => void 0)
		};
		this.pending.set(askId, relay);
		const fromDiscord = new Promise((resolve, reject) => {
			this.waiters.set(askId, {
				resolve,
				reject
			});
		});
		const onAbort = () => {
			...
			waiter.reject(/* @__PURE__ */ new Error("question aborted"));
		};
		request.signal?.addEventListener("abort", onAbort, { once: true });
		try {
			for (const [index, item] of request.questions.entries()) {
				const message = await this.options.rest.createComponentMessage(channelId, formatQuestionContent(item, index, request.questions.length), buildComponents(askId, index, item));
	...
	*/
	async handleInteraction(interaction) {
		...
		if (parsed.kind === "cancel") {
			this.settle(parsed.rpcId, void 0);
			await this.options.rest.respondInteraction(interaction.id, interaction.token, {
				type: DISCORD.CALLBACK_UPDATE_MESSAGE,
				data: {
		...
		if (relay.answers.size === relay.questions.length) this.settle(parsed.rpcId, [...relay.answers.entries()].sort(([a], [b]) => a - b).map(([, value]) => value));
		return true;
	}
	/**
	...
	*/
	settle(askId, answers) {
		const waiter = this.waiters.get(askId);
		if (waiter === void 0) return;
		this.waiters.delete(askId);
		this.pending.delete(askId);
		if (answers === void 0) {
			waiter.reject(/* @__PURE__ */ new Error("the user cancelled the question from Discord"));
			return;
		}
...
function anchors(baseUrl) {
	...
	if (baseUrl !== void 0) list.push(baseUrl);
...
*/
async function loadOptional(specifier, baseUrl) {
...
*/
async function loadHostModules(baseUrl, warn = () => {}) {
	const [llm, agent] = await Promise.all([loadOptional("@deepseek-ai/dsh-llm", baseUrl), loadOptional("@deepseek-ai/dsh-agent", baseUrl)]);
	const modules = {};
	const createUser = llm?.createUserMessage;
	if (typeof createUser === "function") modules.createUserMessage = createUser;
	else warn("@deepseek-ai/dsh-llm createUserMessage is unavailable; using the built-in message builder");
	const install = agent?.installModelSelection;
	if (typeof install === "function") modules.installModelSelection = install;
	else warn("@deepseek-ai/dsh-agent installModelSelection is unavailable; sessions keep their creation-time model");
	return modules;
}
//#endregion
//#region src/index.ts
	...
	ctx.effect(() => {
		...
		(async () => {
			...
			const bridge = new SessionBridge({
				ctx,
				host: await loadHostModules(ctx.baseUrl, (text) => {
					logger.warn(`discord-bridge: ${text}`);
				}),
				store,
				config: {
					cwd: config.cwd,
			...
			disposeQuestions = ctx.on("user-questions/request", async (request, next) => await questionSeat.handleAsk(request, next), { prepend: true });
