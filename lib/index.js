...
*/
function capabilityNotice(maxUploadBytes) {
	...
	*/
	...
	injectNoticeOnce(agent) {
		...
		agent.inject(this.host.createUserMessage({
			content: [{
				type: "text",
				text: capabilityNotice(this.config.maxUploadBytes)
			}],
			source: {
				kind: "plugin",
...
const MCP_TOOL = {
	name: "discord_notify",
	description: "Send a proactive Discord message from this dsh deployment's bot. Defaults to a DM to the deployment owner; pass userId for another allowlisted user's DM, or channelId for a guild channel the bot can post in. Use for monitoring alerts, reminders, and task-completion notifications.",
	inputSchema: {
		type: "object",
		properties: {
			content: {
...
async function handleMcpMessage(message, deps) {
	...
	switch (method) {
		...
		case "tools/list": return respond({ tools: [MCP_TOOL] });
		case "tools/call": {
		...
		(async () => {
			...
			if (config.notifyEnabled && webServer !== void 0) {
				...
				const handler = createNotifyHandler(secret, {
					notifier: new DiscordNotifier(rest, config.allowedUsers[0], config.maxChunksPerReply),
					botLabel: () => botLabel,
					version: "0.5.0"
				}, (level, text) => {
					logger[level](`discord-bridge: ${text}`);
